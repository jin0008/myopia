 ...
 * 병원 관리자가 직접 쓰는 소식.
 *
 * API가 로그인한 관리자의 병원에서 프로필을 찾으므로 이 컴포넌트는 프로필
 * id를 다루지 않는다 — 자기 병원 소식 외에는 손댈 수 없다.
 * PartnerNoticesEditor와 화면은 같고 호출하는 API만 다르다.
 */
export function OwnHospitalNoticesEditor() {
  ...
  const add = async () => {
    ...
    setErr(null);
    try {
      await createOwnHospitalNotice(draft);
      setDraft(EMPTY);
      reload();
    } catch (e: any) {
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <label style={check}>
                  <input
                    ...
                    checked={n.pinned}
                    onChange={async (e) => {
                      await updateOwnHospitalNotice(n.id, { pinned: e.target.checked });
                      reload();
                    }}
                  />
                  고정
                ...
                <label style={check}>
                  <input
                    ...
                    checked={n.published}
                    onChange={async (e) => {
                      await updateOwnHospitalNotice(n.id, { published: e.target.checked });
                      reload();
                    }}
                  />
                  노출
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    await deleteOwnHospitalNotice(n.id);
                    reload();
                  }}
                  style={delBtn}
                >
